...
if ( ! class_exists( 'WC_Product_Data_Fields' ) ) {
	...
	class WC_Product_Data_Fields {
		...
		public function __construct() {
			$this->plugin_prefix 		= 'wc_productdata_options_';
			$this->plugin_basefile 		= plugin_basename( __FILE__ );
			$this->plugin_url 			= NN_WCBS_URI . '/framework/wc-fields/';
			$this->plugin_path 			= NN_WCBS_DIR . '/framework/wc-fields/';
		...
		public function enqueue_scripts() {
			...
			wp_enqueue_style( 'wcpdf-main-css', NN_WCBS_URI . 'framework/wc-fields/assets/css/wcpdf-main.css', array(), '1.0.2' );
			wp_enqueue_style( 'wp-color-picker' );
			wp_enqueue_script( 'wcpdf-main-js', NN_WCBS_URI . 'framework/wc-fields/assets/js/wcpdf-main.js', array('jquery', 'wp-color-picker', 'jquery-ui-datepicker'), '', true );
		...
		public function wc_cpdf_fields() {
			$data = array();
			return apply_filters( 'wc_cpdf_init', $data );
			...
			foreach ( $fields as $key => $fields_array ) {
				...
				foreach ( $fields_array as $field ) {
					if ( isset( $field['tab_name'] ) && $field['tab_name'] != '' ) {
						$href 	= "#".$key;
						$class 	= isset( $field['wrapper_class'] ) ? $field['wrapper_class'] : ''; 
						echo '<li class="'.$key.' '.$class.'"><a href="'.$href.'">'.$field['tab_name'].'</a></li>';
					}
				}
				...
				foreach( $available_fields as $available_field ) {
					...
					foreach ( $fields as $key => $fields_array ) {
						...
						foreach ( $fields_array as $field ) {
							...
							if ( ! array_key_exists( 'tab_name', $field ) ) {

								$this->wc_product_data_options_fields( $field );
						...
						<div id="image-uploader-meta-box" class="image-field-upload">
							...
							<div class="preview-image-wrapper">
								...
								<?php if ( $saved_image ) :?>
									...
									<a href="#" class="remove_image wcpdf-remove-image"><em><?php echo esc_html__(  'Remove', 'wcbs' ); ?></em></a>
							...
							<a class="wcpdf-uppload-image button" href="#" data-uploader-title="<?php echo esc_html__( 'Choose image', 'wcbs' ) ?>" data-uploader-button-text="<?php echo esc_html__( 'Choose image', 'wcbs') ?>"><?php echo esc_html__( 'Choose image', 'wcbs' ) ?></a>
				...
				?>
					...
					<div class="image-field-wrapper gallery form-field">
						...
						<div id="image-uploader-meta-box" class="image-field-upload">
							...
							<input class="wcpdf_image_url" type="hidden" name="wcpdf_image_url_<?php echo $field['id']; ?>" value="<?php echo ($saved_image) ? $saved_image_url[0] : ''; ?>" />
							<a class="wcpdf-uppload-image-gallery button" href="#" data-uploader-title="<?php echo esc_html__( 'Choose images', 'wcbs') ?>" data-uploader-button-text="<?php echo esc_html__( 'Choose images', 'wcbs') ?>"><?php echo esc_html__( 'Choose images', 'wcbs') ?></a>
