 ...
 */
	if ( ! class_exists('Livre_Banner_Image')) {
	
		class Livre_Banner_Image extends WP_Widget {
			...
			public function __construct() {
				parent::__construct(
					'livre_Banner_Image', // Base ID
					esc_html__( 'Tokoo - Banner Image / Advertisement', 'livre' ), // Name

					array(
						'classname' => 'widget-advertisement no-padding ads-block',
						'description' => esc_html__( 'A widget to display banner image attachment', 'livre' )
					) // Args
				);
			}
				...
				?>
					...
					<div class="ads-wrapper">
						<a href="<?php echo esc_url( $instance['link'] ); ?>">
							<img src="<?php echo esc_url( livre_resize( $image_uri, $img_width, $img_height ) ); ?>" width="<?php echo esc_attr( $img_width ); ?>" height="<?php echo esc_attr( $img_height ); ?>" alt="<?php echo esc_attr( $title ); ?>" />
						</a>
					</div>
			...
			public function form( $instance ) {
				...
				if ( isset( $instance[ 'title' ] ) ) {
					...
					$title = esc_html__( 'New title', 'livre' );
				}
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'livre' ); ?></label><br />
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php esc_html_e( 'Link', 'livre' ); ?></label><br />
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php esc_html_e( 'Size', 'livre' ); ?></label><br />
					...
					<?php esc_html_e( 'Set the banner image size <code>Ex: 406x338</code>', 'livre' ); ?>
				</p>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'image_uri' ) ); ?>"><?php esc_html_e( 'Image', 'livre' ); ?></label><br />
				...
				<p>
					<input type="button" value="<?php esc_html_e( 'Upload Image', 'livre' ); ?>" class="button custom_media_upload" id="custom_image_uploader"/>
				</p>
				<?php
			}
...
if ( ! function_exists( 'Livre_Banner_Image_init' ) && class_exists( 'Livre_Banner_Image' ) ) {
	function Livre_Banner_Image_init() {
		register_widget('Livre_Banner_Image');
	}
	add_action('widgets_init', 'Livre_Banner_Image_init');
}