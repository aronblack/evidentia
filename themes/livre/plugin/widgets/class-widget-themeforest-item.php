...
/*-----------------------------------------------------------------------------------*/
	if ( ! class_exists('Livre_ThemeForest_Item')) {
		class Livre_ThemeForest_Item extends WP_Widget {
			...
			function __construct() {
				...
				$widget_ops = array(
					...
					'description' 	=> esc_html__( 'Display ThemeForest items with this widget', 'livre' )
				);
				...
				parent::__construct( 'livre_ThemeForest_Item', esc_html__( 'Tokoo - Themeforest Item', 'livre' ), $widget_ops, $control_ops );
				...
				$this->defaults = array(
					...
					'more_link_txt' => esc_html__( 'View more', 'livre' ),
					'order' 		=> 'desc',
					'target' 		=> '_blank',
					'exclude' 		=> ''
				);
				...
				$this->defaults = apply_filters( 'livre_tf_widget_modify_defaults', $this->defaults );
			}
			...
			function widget( $args, $instance ) {
				...
				<?php
					if ( ! empty( $instance['more_link_url'] ) ) :
						$more_text = isset( $instance['more_link_txt'] ) && ! empty( $instance['more_link_txt'] ) ? $instance['more_link_txt'] : esc_html__( 'View more', 'livre' ); ?>
			...
			function form($instance) {
				...
				esc_html_e( 'Description', 'livre' ); ?>:</label>
					<textarea id="<?php
				...
				<p>
					<label"><?php
				esc_html_e( 'Item categories to show', 'livre' ); ?>:</label><br/>
					<?php
			  ...
			  <p>
					<label"><?php
				esc_html_e( 'Select items from', 'livre' ); ?>:</label><br/>
					<input id="<?php
				...
				esc_html_e( 'Popular Items (WordPress Only)', 'livre' ); ?></label><br/>
					<input id="<?php
				...
				esc_html_e( 'Latest Items', 'livre' ); ?></label><br/>
					<input id="<?php
				...
				esc_html_e( 'Specific User(s)', 'livre' ); ?></label>
			  </p>

				<p>
					<label for="<?php
				echo esc_attr( $this->get_field_id( 'user' ) ); ?>"><?php
				esc_html_e( 'ThemeForest username(s)', 'livre' ); ?>:</label>
					<input id="<?php
				...
				esc_html_e( 'For multiple users, separate by comma: i.e. user1,user2,user3', 'livre' ); ?></small>
				</p>
				...
				esc_html_e( 'Number of items to show', 'livre' ); ?>:</label>
					<input id="<?php
				...
				<p>
					<label><?php
				esc_html_e( 'Order by', 'livre' ); ?>:</label>
						<select id="<?php
				...
				esc_html_e( 'Ascending', 'livre' ); ?></label>
					<input id="<?php
				...
				esc_html_e( 'Descending', 'livre' ); ?></label>
				</p>

				<p>
					<label for="<?php
				echo esc_attr( $this->get_field_id( 'exclude' ) ); ?>"><?php
				esc_html_e( 'Exclude item(s)', 'livre' ); ?>:</label>
					<input id="<?php
				...
				echo strip_tags($instance['exclude']); ?>" class="widefat" />
				  <small class="howto"><?php
				esc_html_e( 'Specify item ID to exclude specific item (separate by comma for multiple items): i.e. 8134834,7184572', 'livre' ); ?></small>
				</p>

				<p>
					<label for="<?php
				echo esc_attr( $this->get_field_id( 'ref' ) ); ?>"><?php
				esc_html_e( 'Referral user', 'livre' ); ?>:</label>
					<input id="<?php
				...
				echo strip_tags( $instance['ref'] ); ?>" class="widefat" />
					<small class="howto"><?php
				esc_html_e( 'Specify username if you want to use items as ThemeForest affiliate links', 'livre' ); ?></small>
				</p>

				<p>
					<label for="<?php
				echo esc_attr( $this->get_field_id( 'more_link_url' ) ); ?>"><?php
				esc_html_e( 'More link URL', 'livre' ); ?>:</label>
					<input id="<?php
				...
				echo esc_attr($instance['more_link_url']); ?>" class="widefat" />
					<small class="howto"><?php
				esc_html_e( 'Specify URL if you want to show "more" link under the items list', 'livre' ); ?></small>
				</p>

				<p>
					<label for="<?php
				echo esc_attr( $this->get_field_id( 'more_link_txt' ) ); ?>"><?php
				esc_html_e( 'More link text', 'livre' ); ?>:</label>
					<input id="<?php
				...
				esc_html_e( 'Open items in', 'livre' ); ?>: </label>
					<select id="<?php
				...
				esc_html_e( 'New Window', 'livre' ); ?></option>
						<option value="_self" <?php selected( '_self', $instance['target'] ); ?>><?php
				esc_html_e( 'Same Window', 'livre' ); ?></option>
					</select>
				</p>
...
if ( ! function_exists( 'Livre_ThemeForest_Item_init' ) && class_exists( 'Livre_ThemeForest_Item' ) ) {
	function Livre_ThemeForest_Item_init() {
		register_widget('Livre_ThemeForest_Item');
	}
	add_action('widgets_init', 'Livre_ThemeForest_Item_init');
}