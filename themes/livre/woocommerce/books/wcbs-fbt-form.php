...
foreach( $products as $current ) {
	...
	$id = $current->get_type() == 'variation' ? $current->variation_id : $current->get_id();

	if ( $index > 0 )
		$thumbs .= '<td class="image_plus image_plus_' . $index . '" data-rel="offeringID_' . $index . '">+</td>';
		$thumbs .= '<td class="image-td" data-rel="offeringID_' . $index . '"><a href="' . get_permalink( $current->get_id() ) . '">' . $current->get_image( 'shop_catalog' ) . '</a></td>';
	...
	<li class="wcbs-wfbt-item">
		<label for="offeringID_<?php echo esc_attr( $index ); ?>">
			...
			<span class="product-name">
				<?php echo ! $index ? '<span class="current_item">'. esc_html__( 'This Item : ', 'livre' ). '</span>' . $current->get_title() : $current->get_title(); ?>
			</span>

			<?php

			if ( $current->get_type() == 'variation' ) {
...
<div class="wcbs-fbt-section">
	<div class="section-header">
		...
		<?php else : ?>
			<h3 class="section-title"><?php esc_html_e( 'Frequently Bought Together', 'livre' ); ?></h3>
		<?php endif; ?>
	</div>
